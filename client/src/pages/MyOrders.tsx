...
export default function MyOrders() {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      default:
        return <Clock className="w-5 h-5 text-slate-600 dark:text-slate-300" />;
    }
  };
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      default:
        return "bg-slate-100 text-slate-800 border-slate-200 dark:border-[#2D4A77]";
    }
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 via-white to-[#F7F4EF]/20 dark:bg-[#0B1120] dark:bg-[#0B1120]">
        <div className="flex items-center gap-3">
          ...
          <span className="text-slate-600 dark:text-slate-300">Loading your orders...</span>
        </div>
      </div>
    );
  }
        ...
        <Card className="max-w-md">
          <CardContent className="p-8 text-center space-y-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-slate-600 dark:text-slate-300">Please sign in to view your order history.</p>
            </div>
        ...
        <div className="container mx-auto px-4 max-w-4xl">
          ...
          <Card className="border-2 border-slate-200 dark:border-[#2D4A77] dark:border-[#2D4A77] shadow-xl">
            <CardContent className="p-8 space-y-6">
              {/* Order Header */}
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 pb-6 border-b-2 border-slate-200 dark:border-[#2D4A77]">
                <div>
                  <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-2">Order Details</h1>
                  <p className="text-slate-600 dark:text-slate-300">Order #{orderDetails.orderNumber}</p>
                  ...
                  <div key={index} className="flex gap-4 p-4 rounded-xl bg-slate-50 border-2 border-slate-200 dark:border-[#2D4A77]">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <span className="text-sm text-slate-600 dark:text-slate-300">Qty: {item.quantity}</span>
                        <span className="font-semibold text-slate-900 dark:text-white">{formatPrice(item.totalInCents)}</span>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              {/* Order Summary */}
              <div className="space-y-3 pt-6 border-t-2 border-slate-200 dark:border-[#2D4A77]">
                ...
                <div className="flex justify-between text-xl font-bold text-slate-900 dark:text-white pt-3 border-t-2 border-slate-200 dark:border-[#2D4A77]">
                  <span>Total</span>
              ...
              {/* Shipping Address */}
              <div className="pt-6 border-t-2 border-slate-200 dark:border-[#2D4A77]">
                ...
                <div className="p-4 rounded-xl bg-slate-50 border-2 border-slate-200 dark:border-[#2D4A77]">
                  ...
                  <p className="text-slate-600 dark:text-slate-300">{orderDetails.shippingAddress1}</p>
                  {orderDetails.shippingAddress2 && (
                    <p className="text-slate-600 dark:text-slate-300">{orderDetails.shippingAddress2}</p>
                  )}
                  <p className="text-slate-600 dark:text-slate-300">
                    {orderDetails.shippingCity}, {orderDetails.shippingState} {orderDetails.shippingZipCode}
                  </p>
                  <p className="text-slate-600 dark:text-slate-300">{orderDetails.shippingCountry}</p>
                  {orderDetails.shippingPhone && (
                    <p className="text-slate-600 dark:text-slate-300 mt-2">{orderDetails.shippingPhone}</p>
                  )}
                </div>
              </div>
            </CardContent>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-[#F7F4EF]/20 py-12 dark:bg-[#0B1120]">
      <div className="container mx-auto px-4 max-w-6xl">
        <div className="mb-8">
          ...
          <p className="text-slate-600 dark:text-slate-300">View and track your Optibio orders</p>
        </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-1">You have referral credits!</h3>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-1">Refer friends, earn $10 credit</h3>
                    <p className="text-sm text-slate-600 dark:text-slate-300">
        ...
        {!orders || orders.length === 0 ? (
          <Card className="border-2 border-slate-200 dark:border-[#2D4A77] dark:border-[#2D4A77] shadow-xl">
            <CardContent className="p-12 text-center space-y-6">
              ...
              <div className="space-y-2">
                ...
                <p className="text-slate-600 dark:text-slate-300">You haven't placed any orders yet. Start shopping to see your orders here!</p>
              </div>
              <Button
          ...
          <div className="space-y-4">
            {orders.map((order: any) => (
              <Card key={order.id} className="border-2 border-slate-200 dark:border-[#2D4A77] dark:border-[#2D4A77] shadow-lg hover:shadow-xl transition-shadow">
                <CardContent className="p-6">
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        <h3 className="text-lg font-bold text-slate-900 dark:text-white">Order #{order.orderNumber}</h3>
                      ...
                      <p className="text-sm text-slate-600 dark:text-slate-300 dark:text-slate-300 mb-1">
